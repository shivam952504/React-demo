...
// GREEN (dark or light) = positive → confidence_score >= 0.8
const isPositive = (item) => item.confidence_score >= 0.8;
...
const positive = items.filter((item) => isPositive(item)).length;
...
// ── Exported boxes: used in Result.jsx, shown ONLY on Attributes Checklist tab ──
...
// ── Default export: collapsible checklist ────────────────────────────────────
...
},
{
  ...
  children: (
    <>
      {/* Boxes centered ABOVE the checklist, inside the tab */}
      <AgentSummaryBoxes
...
```
  {/* Result title — plain, no boxes beside it */}
  <Title level={2} style={{ marginBottom: 16 }}>Result</Title>
  ...
  {/* Confidence legend only on checklist tab */}
