...
year:         CURRENT_YEAR,
month:        CURRENT_MONTH,   // single select → string
geo:          [“ALL”],
program:      [“ALL”],
lob:          [“ALL”],
supervisor:   [“ALL”],
...
year_type:    “Calendar Year”,
year:         CURRENT_YEAR,
month:        CURRENT_MONTH,   // send current month, not ALL
geo:          [“ALL”],
program:      [“ALL”],
lob:          [“ALL”],
supervisor:   [“ALL”],
...
/* trigger debounce whenever filters change */
useEffect(() => {
if (debounceRef.current) clearTimeout(debounceRef.current);
debounceRef.current = setTimeout(() => {
setDebouncedFilters((prev) => ({ …filters }));
}, 600);
...
year:        CURRENT_YEAR,
month:       CURRENT_MONTH,   // string
geo:         [“ALL”],
program:     [“ALL”],
lob:         [“ALL”],
supervisor:  data.supervisor?.[0] ? [data.supervisor[0]] : [“ALL”],
tenure_units: (data.tenure_units ?? data.tenure_unit)?.[0] ? [(data.tenure_units ?? data.tenure_unit)[0]] : [“ALL”],
}));
...
/* ── load data — fires only after 600ms debounce ────────*/
useEffect(() => {
if (!isFilterLoaded) return;
if (!debouncedFilters) return;   // wait for first debounce tick
const thisViewBy = viewBy;
setLoading(true);
setError(null);
axios.post(API_BASE + dataEndpoint(thisViewBy), debouncedPayload)
.then((res) => {
if (activeViewByRef.current !== thisViewBy) return;
...
.finally(() => { if (activeViewByRef.current === thisViewBy) setLoading(false); });
}, [debouncedFilters, viewBy, isFilterLoaded]);
