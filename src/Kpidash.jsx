...
/* ── NEW state: infoTile holds tile object for (i) modal ── */
...
/* row 4: SVG chart — EXACTLY as original PDF pages 14-15 */
const VW=280,VH=GH,xAW=34,yAW=14;
const cW=VW-yAW;
...
return(
  <div className="kpi-tile" key={label}>
    ...
    {/* row 1: title + (i) button above target — CHANGED from original */}
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
      {/* left: label — unchanged */}
      <div style={{
        fontSize:12,fontWeight:700,color:"#475569",flex:1,marginRight:8,lineHeight:1.4,
        whiteSpace:"normal",overflow:"hidden",display:"-webkit-box",
        WebkitLineClamp:2,WebkitBoxOrient:"vertical"
      }}>{label}</div>

      {/* right: (i) on top, target below it */}
      <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",flexShrink:0,gap:4}}>
        {/* (i) info button — NEW, clicking opens info modal */}
        <div
          style={{
          ...
          onClick={e=>{e.stopPropagation();setInfoTile(tile);}}
        >i</div>

        {/* target — moved below (i) */}
        {target!=="-"&&(
    ...
    {/* row 4: SVG chart — original logic exactly, + value labels on bars */}
    <div style={{width:"100%",minWidth:0,marginTop:6}}>
      {hasG?(
        <svg viewBox={`0 0 ${VW} ${VH+xAW}`}
          ...
          {/* Bars + NEW: value label on top of each bar */}
          {nums.map((pt,i)=>{
    ...
    {/* row 5: current trend + bonus/target trend from backend — UNCHANGED */}
...
/* ====================================================================================
INFO MODAL — opens when (i) is clicked, shows bigger chart + stats
==================================================================================== */
function InfoModal(){
if(!infoTile)return null;
const{label,color,target,series,dates:gd,viewBy:tvb,cms,trend}=infoTile;
const vb=tvb||viewBy;
const total=gd.length;
const targetNum=parseFloat(String(target).replace(”%”,””));

```
const nums=series.map(pt=>({y:(pt.y!==null&&!isNaN(Number(pt.y)))?Number(pt.y):null}));
const valid=nums.filter(p=>p.y!==null).map(p=>p.y);
const hasG=valid.length>0;
...
const yMid=Math.round(yMax/2);
const yRange=yMax;
const VW=560,VH=180,xAW=34,yAW=22;
const cW=VW-yAW;
const barGap=cW/total;
...
const tPct=(!isNaN(targetNum)&&yRange>0)?Math.min((targetNum/yRange)*100,100):null;
const tY=tPct!==null?VH*(1-tPct/100):null;
const bl=v=>Math.max((v/yRange)*VH,2);
  ...
  >
    ...
    >
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
        <div>
          <div style={{fontSize:17,fontWeight:800,color:"#1e293b",marginBottom:6}}>{label}</div>
          <div style={{display:"flex",gap:16,flexWrap:"wrap",alignItems:"center"}}>
            ...
            <span style={{fontSize:12,color:"#64748b"}}>Period: <b>{vb}</b></span>
            <span style={{fontSize:12,color:"#64748b"}}>Data Points: <b>{total}</b></span>
          </div>
        </div>
        {/* ── close ✕ button ── */}
        <button
          ...
          style={{
            ...
            background:"#f1f5f9",fontSize:16,cursor:"pointer",
            display:"flex",alignItems:"center",justifyContent:"center",
      ...
      <div style={{display:"flex",gap:16,marginBottom:16,alignItems:"center"}}>
        ...
        {target!=="-"&&(
          <span style={{fontSize:12,color:"#64748b",display:"flex",alignItems:"center",gap:6}}>
            <svg width="20" height="8"><line x1="0" y1="4" x2="20" y2="4" stroke="#3b82f6" strokeWidth="1.5" strokeDasharray="4,2"/></svg>
            Target
          </span>
        )}
      </div>
      ...
      <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap"}}>
        {[
          ...
          <div key={lbl} style={{
            ...
            border:"1px solid #e2e8f0",flex:"1 1 auto",minWidth:90,textAlign:"center"
          }}>
            <div style={{fontSize:10,color:"#94a3b8",fontWeight:600,textTransform:"uppercase",marginBottom:3}}>{lbl}</div>
            <div style={{fontSize:15,fontWeight:800,color:"#1e293b"}}>{val}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{width:"100%",minWidth:0}}>
        {hasG?(
            ...
            {nums.map((pt,i)=>{
              ...
              return(
                <g key={i}>
                  <rect x={x} y={barTop} width={barW} height={h}
                    fill={bColor(pt.y)} rx={1}>
                    <title>{tipLabel}: {pt.y}</title>
                  </rect>
                  {showLbl&&(
                    <text
                      x={x+barW/2} y={barTop-3}
                      textAnchor="middle" fontSize={7}
                      fill="#475569" fontWeight="700" fontFamily="sans-serif"
                    >{pt.y}</text>
                  )}
                </g>
              );
          ...
          <div style={{height:100,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:"#cbd5e1"}}>No Data</div>
        )}
      </div>

      {/* ── footer: current month + trend ── */}
      <div style={{marginTop:20,paddingTop:16,borderTop:"1px solid #f1f5f9",display:"flex",gap:32,flexWrap:"wrap"}}>
        <div>
          <div style={{fontSize:10,color:"#64748b",fontWeight:600,textTransform:"uppercase",marginBottom:4}}>Current Month</div>
        ...
        {trend&&(
          <div>
            <div style={{fontSize:10,color:"#64748b",fontWeight:600,textTransform:"uppercase",marginBottom:4}}>Bonus & Target Trend</div>
            <div style={{fontSize:13,fontWeight:700,color:"#3b82f6"}}>{trend}</div>
          </div>
        )}
      </div>
...
```
  ...
  {/* ── INFO MODAL (opens when (i) is clicked) ──────────────────────── */}
  <InfoModal/>
</>
```
