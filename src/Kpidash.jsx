...
if(viewBy===“quarterly”){
// “Quarter 1”, “Quarter 2” etc. (exact backend format seen in table)
const wordQ=k.match(/^Quarter\s*(\d)/i);if(wordQ)return `Q${wordQ[1]}`;
// “Q1”, “Q1 2026”, “2026-Q1”
const qm=k.match(/Q(\d)/i);if(qm)return `Q${qm[1]}`;
// plain number “1” “2” “3” “4”
const nm=parseInt(k);if(!isNaN(nm)&&nm>=1&&nm<=4)return `Q${nm}`;
// iso month → derive quarter
const iso=k.match(/^\d{4}-(\d{2})$/);
if(iso){const mo=parseInt(iso[1]);return `Q${Math.ceil(mo/3)}`;}
