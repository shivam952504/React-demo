...
return(
  <div className="kpi-tile" key={label}>
    ...
    {/* row 4: graph — SVG-based so bars NEVER break layout */}
    <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"flex-end",minWidth:0}}>
      {hasG?(()=>{
        /* Use SVG so bars are pixel-perfect and can't cause overflow */
        const W=300, H=GH, yAxisW=36, xAxisH=16;
        const chartW=W-yAxisW;
        const barW=Math.max(1, chartW/total - 1);
        const barGap=chartW/total;
        const tY=tPct!==null ? H*(1-tPct/100) : null;
        /* x-axis: show every label for <=15 bars, every other for <=25, every 3rd for more */
        const step=total<=15?1:total<=25?2:total<=31?3:5;
        return(
          <div style={{width:"100%",minWidth:0,overflow:"hidden"}}>
            <svg viewBox={`0 0 ${W} ${H+xAxisH}`} preserveAspectRatio="none"
              style={{width:"100%",height:H+xAxisH,display:"block",overflow:"visible"}}>

              {/* Y axis labels — fixed x position */}
              <text x={yAxisW-4} y={8}   textAnchor="end" fontSize={8} fill="#94a3b8" fontWeight="600">{yMax}</text>
              <text x={yAxisW-4} y={H/2+4} textAnchor="end" fontSize={8} fill="#94a3b8" fontWeight="600">{yMid}</text>
              <text x={yAxisW-4} y={H}   textAnchor="end" fontSize={8} fill="#94a3b8" fontWeight="600">{yMin}</text>

              {/* grid lines */}
              {[0,H/2,H].map((y,i)=>(
                <line key={i} x1={yAxisW} y1={y} x2={W} y2={y} stroke="rgba(148,163,184,0.2)" strokeWidth={0.5}/>
              ))}
              ...
              {nums.map((pt,i)=>{
                ...
                const h=bH(pt.v);
                const x=yAxisW+i*barGap;
                const y=H-h;
                return <rect key={i} x={x} y={y} width={barW} height={h}
                  fill={bColor(pt.v)} rx={1} title={`${pt.v}${unit}`}/>;
              })}

              {/* dashed target line */}
              {tY!==null&&(
                <line x1={yAxisW} y1={tY} x2={W} y2={tY}
              ...
              {gd.map((d,i)=>{
                ...
                const x=yAxisW+i*barGap+barW/2;
                return(
                  <text key={i} x={x} y={H+xAxisH-2}
                    textAnchor="middle" fontSize={7} fill="#94a3b8" fontWeight="600">{lbl}</text>
                );
              })}
            </svg>
          </div>
