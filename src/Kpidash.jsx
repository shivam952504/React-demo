...
```
    ...
    <div style={{display:"flex", gap:10, alignItems:"stretch", marginBottom:14}}>
      ...
      <div className="kpi-filter-row" style={{
        ...
        {/* MONTH — single, day only */}
        {viewBy === "day" && (
          <div className="filter-col">
            <label>MONTH</label>
            <Select value={filters.month} size="small"
              style={{width:"100%"}} popupMatchSelectWidth={false}
              styles={{popup:{minWidth:140}}}
        ...
        <div className="filter-col">
          ...
          <Select mode="multiple" value={filters.geo} size="small"
            ...
            onChange={v => updateMultiFilter("geo", v)}>
        ...
        <div className="filter-col">
          ...
          <Select mode="multiple" value={filters.program} size="small"
            style={{width:"100%"}} popupMatchSelectWidth={false}
            styles={{popup:{minWidth:160}}} maxTagCount="responsive"
            onChange={v => updateMultiFilter("program", v)}>
        ...
        <div className="filter-col">
          ...
          <Select mode="multiple" value={filters.lob} size="small"
            style={{width:"100%"}} popupMatchSelectWidth={false}
            styles={{popup:{minWidth:160}}} maxTagCount="responsive"
            onChange={v => updateMultiFilter("lob", v)}>
        ...
        <div className="filter-col">
          ...
          <Select mode="multiple" value={filters.supervisor} size="small"
            style={{width:"100%"}} popupMatchSelectWidth={false}
            styles={{popup:{minWidth:160}}} maxTagCount="responsive"
            onChange={v => updateMultiFilter("supervisor", v)}>
        ...
        <div className="filter-col">
          ...
          <Select mode="multiple" value={filters.tenure_units} size="small"
            ...
            onChange={v => updateMultiFilter("tenure_units", v)}>
            {(filterOptions.tenure_units||filterOptions.tenure_unit||[]).map(v=>
              <Option key={v} value={v}>{v}</Option>)}
          </Select>
        </div>

        {/* divider */}
        <div style={{width:1, alignSelf:"stretch", background:"#e8e8e8", flexShrink:0, margin:"0 4px"}}/>
      ...
      <div style={{
        ...
        <div className="viewby-seg" style={{display:"flex", gap:0, borderRadius:6, overflow:"hidden", border:"1px solid #d9d9d9"}}>
          {["Day","Week","Month"].map(lbl => {
            const val = lbl.toLowerCase();
            const active = viewBy === val;
            return (
              <div key={val} onClick={() => setViewBy(val)} style={{
                padding:"5px 14px", cursor:"pointer", fontSize:12, fontWeight:600,
                background: active ? "#153a6f" : "#fff",
                color: active ? "#fff" : "#333",
                transition:"all 0.2s",
                borderRight: lbl !== "Month" ? "1px solid #d9d9d9" : "none",
              }}>
                {lbl}
              </div>
            );
    ...
    {/* ═══ TILES — 2 rows × 3, with prev/next in middle ═══*/}
      ...
      <div style={{marginBottom:16}}>
        ...
        <div style={{display:"flex", gap:12, marginBottom:12}}>
          ...
          {Array.from({length: 3 - tileRow1.length}).map((_,i) => (
            <div key={`pad1-${i}`} style={{flex:1}}/>
          ))}
        </div>

        {/* PREV / NEXT between rows */}
        {totalTilePages > 1 && (
          <div style={{
