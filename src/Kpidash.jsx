...
/* ─────────────────────────────────────────────────────────────
getOverallDisplay — extracts the “Overall” value from any field
Returns null for dash / empty / NaN / boolean
──────────────────────────────────────────────────────────────*/
...
/* getCellDisplay — same but returns “-” instead of null (for table cells) */
function getCellDisplay(raw) {
const v = getOverallDisplay(raw);
return v !== null ? v : “-”;
}

/* isNested — true if field is a CSAT-style object with Contractual/Bonus flags */
...
/* getBoolFlag — reads Contractual or “Bonus and Penalty” from nested object */
function getBoolFlag(raw, flag) {
if (!raw || typeof raw !== “object”) return null;
return flag in raw ? raw[flag] === true : null;
}

/* parseDateEntries — works for daily/weekly and monthly grouped shapes */
...
/* discoverMetricKeys — ordered union of all keys across all date entries */
...
/* hasRealValue — at least one non-dash value (used for tiles) */
...
/* ─────────────────────────────────────────────────────────────
metricPassesToggle
contractualOn, bonusOn: boolean toggles
Logic:
both OFF  → always pass (show everything)
C ON  only → nested metrics must have Contractual=true
B ON  only → nested metrics must have “Bonus and Penalty”=true
both ON   → nested metrics must have BOTH true (&&)
Flat (non-nested) metrics always pass regardless.
──────────────────────────────────────────────────────────────*/
function metricPassesToggle(key, dates, dataMap, contractualOn, bonusOn) {
// both OFF → show all
if (!contractualOn && !bonusOn) return true;

// check if this key is a flat field across all dates
const isFlat = !dates.some((d) => isNested(dataMap[d]?.[key]));
if (isFlat) return true; // flat fields always visible

// nested: check flags across all dates
const hasContractualTrue = dates.some(
(d) => getBoolFlag(dataMap[d]?.[key], “Contractual”) === true
);
const hasBonusTrue = dates.some(
(d) => getBoolFlag(dataMap[d]?.[key], “Bonus and Penalty”) === true
);

if (contractualOn && bonusOn) return hasContractualTrue && hasBonusTrue;
if (contractualOn)            return hasContractualTrue;
if (bonusOn)                  return hasBonusTrue;
...
const [allMetricKeys,  setAllMetricKeys]  = useState([]);
...
/* ═══════════════════════════════════════════════════════════
buildDynamicTable — builds allTableRows + allTileData
No filtering here — filtering done in useMemo below
═══════════════════════════════════════════════════════════*/
...
/* ── COLUMNS (date columns, always full — filtering only hides rows) ── */
const cols = [
  {
    title: "Metric",
    dataIndex: "metric",
...
const tiles = activeKeys
  ...
  .map((key, idx) => {
    ...
    // store flags for toggle filtering
    const contractual = dates.some(
      (d) => getBoolFlag(dataMap[d]?.[key], "Contractual") === true
    );
    const bonus = dates.some(
      (d) => getBoolFlag(dataMap[d]?.[key], "Bonus and Penalty") === true
    );
    ...
    return {
      ...
      unit, value: tileValue,
      series, dates: graphDates,
...
/* ═══════════════════════════════════════════════════════════
DERIVED: filtered table rows + tiles based on toggles
Recomputed whenever contractualOn / bonusOn / data changes
═══════════════════════════════════════════════════════════*/
const tableData = useMemo(() => {
if (!contractualOn && !bonusOn) return allTableRows;
return allTableRows.filter((row) =>
metricPassesToggle(row.key, chartDates, chartDataMap, contractualOn, bonusOn)
);
}, [contractualOn, bonusOn, allTableRows, chartDates, chartDataMap]);

const filteredTiles = useMemo(() => {
if (!contractualOn && !bonusOn) return allTileData;
return allTileData.filter((tile) => {
if (tile.flat) return true; // flat fields always visible
if (contractualOn && bonusOn) return tile.contractual && tile.bonus;
if (contractualOn) return tile.contractual;
if (bonusOn)       return tile.bonus;
return true;
});
}, [contractualOn, bonusOn, allTileData]);

/* reset tile page when filtered set changes */
...
.filter-box { display: flex; flex-direction: column; width: 100%; }
.heat-cell {
background: #d7f5e9;
padding: 4px 6px;
border-radius: 4px;
...
.nav-btn:hover:not(:disabled) { background: #f5f5f5; border-color: #aaa; }
...
```
      ...
      .toggle-pill.on { background: #153a6f; }
      .toggle-pill::after {
        content: "";
        position: absolute;
        top: 2px; left: 2px;
      ...
      .divider { height: 1px; background: #f0f0f0; margin: 0 -2px; }
    `}</style>
    ...
    <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
      ...
      <div className="viewby-box">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
          ...
          {/* Contractual toggle */}
          <div
            className={`toggle-btn${contractualOn ? " active" : ""}`}
            onClick={() => setContractualOn((p) => !p)}
          >
            <div className={`toggle-pill${contractualOn ? " on" : ""}`} />
            Contractual
          </div>

          {/* Bonus and Penalty toggle */}
          <div
            className={`toggle-btn${bonusOn ? " active" : ""}`}
            onClick={() => setBonusOn((p) => !p)}
          >
            <div className={`toggle-pill${bonusOn ? " on" : ""}`} />
            Bonus and Penalty
          </div>
        </div>
      </div>
...
/* ═══════════════════════════════════════════════════════════
renderFilter — backend sends options including ALL already
═══════════════════════════════════════════════════════════*/
      ...
      {hasGraph ? (
        <>
          <div style={{ display: "flex", alignItems: "flex-end", gap: 3 }}>
            ...
            {/* BARS — pinned to bottom via alignItems: flex-end */}
            <div style={{
