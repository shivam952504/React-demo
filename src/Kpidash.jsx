...
/* filter values — month/year/year_type are single, rest are arrays */
const [filters, setFilters] = useState({
year_type:    “Calendar Year”,
year:         CURRENT_YEAR,
month:        “January”,       // default January
geo:          [“ALL”],
program:      [“ALL”],
lob:          [“ALL”],
supervisor:   [“ALL”],
...
/* debouncedFilters — only updates 600ms after user stops changing filters
so we don’t fire an API call on every single option click                */
...
/* trigger debounce whenever filters change (600ms pause) */
useEffect(() => {
...
year:        filters.year,
month:       filters.month,        // string
geo:         filters.geo,          // array
program:     filters.program,      // array
lob:         filters.lob,          // array
supervisor:  filters.supervisor,   // array
tenure_units: filters.tenure_units,  // array
}), [filters]);

/* debouncedPayload — always reads latest filters.
Debounce only controls WHEN the effect fires, not WHAT is sent. */
...
tenure_units: filters.tenure_units,
}), [filters]);

/* ══════════════════════════════════════════════════════════
UNIFIED EFFECT — fires on viewBy change (immediate)
Loads filter options first, then loads data.
Both APIs called every time viewBy changes.
══════════════════════════════════════════════════════════*/
useEffect(() => {
...
```
// Build payload from current filters (use January default on first mount)
const filterCallPayload = {
  year_type:    filters.year_type,
  year:         filters.year,
  month:        filters.month,
  geo:          filters.geo,
...
axios.post(API_BASE + filterEndpoint(thisViewBy), filterCallPayload)
  .then((res) => {
...
/* ── load data — fires after 600ms debounce on any filter change ──
Also fires when isFilterLoaded becomes true (after viewBy change) */
useEffect(() => {
...
```
// Call filter API with current debounced values (updates dropdown options)
const currentPayload = {
  year_type:    debouncedFilters.year_type,
  year:         debouncedFilters.year,
  month:        debouncedFilters.month,
  geo:          debouncedFilters.geo,
  program:      debouncedFilters.program,
  lob:          debouncedFilters.lob,
  supervisor:   debouncedFilters.supervisor,
  tenure_units: debouncedFilters.tenure_units,
};

// Fire both in parallel
Promise.all([
  axios.post(API_BASE + filterEndpoint(thisViewBy), currentPayload),
  axios.post(API_BASE + dataEndpoint(thisViewBy),   currentPayload),
])
  .then(([filterRes, dataRes]) => {
    if (activeViewByRef.current !== thisViewBy) return;

    // update filter options from filter response
    if (filterRes.data) setFilterOptions(filterRes.data);

    // update table/tiles from data response
...
}, [debouncedFilters, isFilterLoaded]);
        ...
        <>
          <div style={{ display:"flex", alignItems:"flex-end", gap:3 }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize:11, fontWeight:700, color:"#666", lineHeight:1 }}>{scaleMax}</span>
              <span style={{ fontSize:11, fontWeight:700, color:"#666", lineHeight:1 }}>{Math.round(scaleMax / 2)}</span>
              <span style={{ fontSize:11, fontWeight:700, color:"#666", lineHeight:1 }}>0</span>
            </div>

            {/* BARS + TARGET LINE — relative container so target line overlays bars */}
            <div style={{
              ...
              {/* TARGET DOTTED LINE — absolute, positioned from bottom */}
              {!isNaN(targetNum) && targetNum > 0 && targetNum <= scaleMax && (
                <div
                  title={`Target: ${target}`}
                  style={{
                    position:"absolute",
                    left:0, right:0,
                    bottom: `${(targetNum / scaleMax) * 100}%`,
                    height:0,
