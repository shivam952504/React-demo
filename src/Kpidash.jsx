...
/* ─────────────────────────────────────────────────────────────────
smartSort — sorts date/week/month keys in correct chronological order.

Handles these backend key formats:
Daily   : “2026-01-01”, “2026-01-02”  → ISO string → natural sort works
Weekly  : “2026-W01”, “2026-W02”      → parse year+week number
“W1”, “W2”, “W10”           → parse week number only
“Week 1”, “Week 10”         → parse number after “Week “
Monthly : “2026-01”, “2026-02”        → ISO year-month → natural sort works
“January”, “February”, …  → map to month index
“Jan-2026”, “Feb-2026”      → parse month name + year
──────────────────────────────────────────────────────────────────*/
const MONTH_ORDER = {
...
const va = dateKeyToSortValue(a);
const vb = dateKeyToSortValue(b);
...
// ← smart sort instead of naive .sort()
...
/* stable endpoint helpers — take viewBy as arg, never stale */
...
/* track which viewBy is currently “expected” so stale
responses from a previous viewBy are silently discarded    */
...
setIsFilterLoaded(false);   // block data load until new filters arrive
setLoading(true);

```
axios.post(API_BASE + filterEndpoint(thisViewBy), defaultFilterPayload)
  .then((res) => {
    if (activeViewByRef.current !== thisViewBy) return; // stale, discard
    const data = res.data || {};
  ...
  .finally(() => {
    if (activeViewByRef.current === thisViewBy) setLoading(false);
  });
```
