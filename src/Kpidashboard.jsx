...
const runFetch=(vb,payload)=>{
  ...
  .then(r=>{
    ...
    setFilterOptions(r||{});
    setIsFlReady(true);
    // pick first LOB from filter response — do NOT send data until LOB is known
    const lobList = r?.lob || [];
    const firstLob = lobList.length > 0 ? lobList[0] : payload.lob;
    const dataPayload = {...payload, lob: firstLob};
    // update filters state with correct LOB (silent — won't trigger debFilters fetch because isFlReady was just set)
...
useEffect(()=>{
  ...
  if(!debFilters.lob)return;  // wait until LOB is set by runFetch
  if (viewBy === "daterange") return;
